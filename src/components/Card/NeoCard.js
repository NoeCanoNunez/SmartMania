...
function NewMiniCard({ equipos, queEquiposMostrar }) {
  ...
  const [messageCard, setMessageCard] = useState(
    "Presione Aquí para Añadir un Equipo"
  );
  ...
  const enviarHandler = (e) => {
    ...
    setNamesEquipos([]);
    queEquiposMostrar(
      equipos.filter((equipo) => equipo.Equipo === e.target.innerText)[0]
    );
  };
      ...
      <div className="container__cards">
        ...
        >
          ...
          <div style={{ textAlign: "center", verticalAlign: "middle" }}>
            {inputView ? (
              <form>
                <label id="labelInputCard" htmlFor="inputCard">
                  {messageCard}
                </label>
                <input
                  id="inputCard"
                  ...
                  autoFocus
                />
                <div id="optionsContainer" className={inputView}>
...
function NeoCard({
  ...
  const cambiandoEquiposAMostrar = (prop) =>
    setEquiposCard([...equiposCard, prop]);
      ...
      >
        <div>
          ...
          <div className="container_miniCard">
            ...
            <NewMiniCard
              equipos={dataEquipos}
              queEquiposMostrar={cambiandoEquiposAMostrar}
            />
          </div>
        </div>
      </motion.div>
